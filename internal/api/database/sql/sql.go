...
func (d *DAO) GetAccessTokenBySHA(sha string) ([]modelV1.AccessToken, error) {
	...
	sb.Select("*").From(d.generateCompleteTableName(tableAccessToken)).Where(
		sb.Equal("token_last_eight", sha),
	)
	query, args := sb.Build()
	...
	for rows.Next() {
		var rowJSON modelV1.AccessToken
		if scanErr := rows.Scan(&rowJSON); scanErr != nil {
			return nil, scanErr
		}
		result = append(result, rowJSON)
	}
...
func (d *DAO) createPersesAccessTokenTable() string {
	return sqlbuilder.CreateTable(d.generateCompleteTableName(tableAccessToken)).IfNotExists().
		...
		Define("token_hash", "VARCHAR(64)", "UNIQUE"). // sha256 is 64 chars in hex
		Define("token_salt", "VARCHAR(64)", "NOT NULL").
		Define("token_last_eight", "VARCHAR(8)", "NOT NULL").
		Define("created_unix", "TIMESTAMP", "NOT NULL").
		Define("updated_unix", "TIMESTAMP", "NOT NULL").
		Define("has_used", "BOOLEAN", "NOT NULL DEFAULT false").
		Define("exp_date", "TIMESTAMP", "NOT NULL").
